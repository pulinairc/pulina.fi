 ...
 */
function pulina-2014_wp_title( $title, $sep ) {
	if ( is_feed() ) {
		return $title;
	}
	...
	if ( $paged >= 2 || $page >= 2 ) {
		$title .= " $sep " . sprintf( __( 'Page %s', 'pulina-2014' ), max( $paged, $page ) );
	}

	return $title;
}
add_filter( 'wp_title', 'pulina-2014_wp_title', 10, 2 );
...
load_theme_textdomain( 'pulina-2014', get_template_directory() . '/languages' );

/**
 * Scriptit ja tyylit.
 */
function pulina-2014_scripts() {
	...
	wp_enqueue_style( 'pulina-2014-style', get_stylesheet_uri() );
	...
	wp_enqueue_script( 'pulina-2014-skip-link-focus-fix', get_template_directory_uri() . '/js/skip-link-focus-fix.js', array(), '20130115', true );
...
add_action( 'wp_enqueue_scripts', 'pulina-2014_scripts' );
...
register_nav_menus( array(
	'primary' => __( 'Primary Menu', 'pulina-2014' ),
) );
