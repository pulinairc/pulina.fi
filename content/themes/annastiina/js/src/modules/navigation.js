...
(function ($) {
  ...
  $('.menu-item a, .dropdown button').on('keyup', function () {
    if ($('.dropdown').find(':focus').length !== 0) {
      ...
      if (event.keyCode === 27) {
        ...
        // jscs:enable
        screenReaderSpan.text(annastiina_screenReaderText.expand);
    ...
    if (window.innerWidth > responsivenav) {
      ...
      screenReaderSpanPrev.text(annastiina_screenReaderText.expand);
      ...
      screenReaderSpanNext.text(annastiina_screenReaderText.expand);
    }
  });
  ...
  // TODO: Rather fix the issue: https://github.com/digitoimistodude/annastiina/issues/26
  // $(() => {
  //   if (window.innerWidth < responsivenav) {
  ...
  //     dropdownItem.attr('aria-label', annastiina_screenReaderText.collapse);
  //     screenReaderSpan.text(annastiina_screenReaderText.collapse);
  //   }
  // });

  // Add default dropdown-toggle label
  $('.dropdown-toggle').each(function () {
    $(this).attr('aria-label', `${annastiina_screenReaderText.expand_for} ${$(this).prev().text()}`);
  });
  ...
  siteHeaderMenu.find('.dropdown-toggle').click(function (e) {
    if (enterPressed || window.innerWidth < responsivenav) {
      ...
      $(this).attr(
        'aria-label',
        $(this).attr('aria-label') === `${annastiina_screenReaderText.collapse_for} ${$(this).prev().text()}`
          ? `${annastiina_screenReaderText.expand_for} ${$(this).prev().text()}`
          : `${annastiina_screenReaderText.collapse_for} ${$(this).prev().text()}`,
      );
    }
  });
